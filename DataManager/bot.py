...
class MyBot:
    ...
    async def command_start_handler(self, message: Message) -> None:
        ...
        try:
            if user_id == self.admin_id:
                ...
                await message.answer("Seleziona un elemento:", reply_markup=keyboard)
    ...
    async def handle_message(self, message: types.Message):
        ...
        try:
            if user_id == self.admin_id:
                    ...
                    await message.answer(f"File salvato in {file_path}")
    ...
    async def process_callback_menage_page(self, callback_query: types.CallbackQuery):
        try:
            ...
            await self.bot.edit_message_text("Seleziona un elemento:", callback_query.from_user.id, callback_query.message.message_id, reply_markup=keyboard)
        except Exception as ex:
            logger_config.logger.error(f"Errore durante l'esecuzione di command_start_handler: {ex}", exc_info=True)
            await self.bot.send_message(self.admin_id, f"{ex}")  
    ...
    async def handle_folder_callback(self, callback_query: types.CallbackQuery):
        try:       
            ...
            keyboard = self.folder_manager.list_entries(None, 0)
            await callback_query.message.edit_text("Seleziona un elemento:", reply_markup=keyboard)
        except Exception as ex:
            logger_config.logger.error(f"Errore durante l'esecuzione di command_start_handler: {ex}", exc_info=True)
            await self.bot.send_message(self.admin_id, f"{ex}")  
    ...
    async def handle_cancel_callback(self, callback_query: types.CallbackQuery):
        try:
            ...
            await callback_query.message.edit_text("Operazione annullata")
        except Exception as ex:
            logger_config.logger.error(f"Errore durante l'esecuzione di command_start_handler: {ex}", exc_info=True)
            await self.bot.send_message(self.admin_id, f"{ex}")  
    ...
    async def handle_action_file_callback(self, callback_query: types.CallbackQuery, state: FSMContext):
        try:
            ...
            await callback_query.message.edit_text(f"Azioni per il file {file_name}:", reply_markup=keyboard)
        except Exception as ex:
            logger_config.logger.error(f"Errore durante l'esecuzione di command_start_handler: {ex}", exc_info=True)
            await self.bot.send_message(self.admin_id, f"{ex}")  
    ...
    async def handle_action_folder_callback(self, callback_query: types.CallbackQuery, state: FSMContext):
        try:
            ...
            await callback_query.message.edit_text(f"Azioni per la cartella {folder_name}:", reply_markup=keyboard)
        except Exception as ex:
            logger_config.logger.error(f"Errore durante l'esecuzione di command_start_handler: {ex}", exc_info=True)
            await self.bot.send_message(self.admin_id, f"{ex}")         
    ...
    async def handle_enter_folder_callback(self, callback_query: types.CallbackQuery):
        try:
            ...
            if self.folder_manager.change_folder(folder_name):
                ...
                await callback_query.message.edit_text("Impossibile accedere alla cartella selezionata")
        except Exception as ex:
            logger_config.logger.error(f"Errore durante l'esecuzione di command_start_handler: {ex}", exc_info=True)
            await self.bot.send_message(self.admin_id, f"{ex}")  
